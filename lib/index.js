...
function resolvePath(path) {
  return new Promise(function (resolve, reject) {
    try {
      ...
      var directoryPath = directory.dir;
      if (!directory.ext) {
        directoryPath += pathModule.sep + directory.base;
      }
      var pathToModuleP = findModuleDirectory(directoryPath);
      pathToModuleP.then(function (pathToModule) {
        return moduleName(pathToModule);
      }).then(getModulePath.bind(null, path)).then(resolve).catch(reject);
    } catch (err) {
      reject(err);
    }
  });
...
function getModulePath(fullpath, moduleName) {
  return pathModule.normalize(fullpath.substr(fullpath.lastIndexOf(moduleName)));
}
